...
std::shared_ptr<Fraction> DoSomething(int firstNum, int secondNum) {
	for (int i = 2; i < 17; i++) {
	...
	return std::make_shared<Fraction>(firstNum, secondNum);
}
...
int main() {
	std::shared_ptr<Fraction> ptr = std::make_shared<Fraction>(49, 11711);
